  ...
  return (
    <div className="min-h-screen bg-white py-20 px-4 sm:px-6 lg:px-8 flex justify-center">
      <div className="w-full max-w-4xl space-y-8">
        <h1 className="text-2xl md:text-3xl font-bold mb-6 text-center md:text-left">로그인</h1>
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col space-y-4">
            ...
            <div className="flex items-center mt-4">
              ...
              />
              <label htmlFor="saveId" className="text-sm text-gray-700 cursor-pointer select-none">
                아이디 저장
              </label>
            </div>
            ...
            <button
              ...
              className={`mt-6 w-full py-3 rounded-full font-semibold transition ${
                isDisabled || loading ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-black text-white hover:bg-gray-800 cursor-pointer"
              }`}
            >
              {loading ? "로그인 중..." : "로그인"}
            </button>

            <p onClick={() => navigate("/reset-password")} className="mt-4 text-violet-600 text-sm cursor-pointer hover:underline">
              비밀번호를 잊으셨나요?
            </p>
          </div>
        ...
        {showSocialModal && (
          <div className="fixed inset-0 bg-opacity-40 backdrop-blur-md flex justify-center items-center z-50">
            <div className="bg-white rounded-2xl shadow-lg p-6 w-80">
              <h2 className="text-lg font-semibold mb-4 text-center">소셜 계정을 선택하세요</h2>
              <div className="flex flex-col space-y-3">
                <button disabled={loading} onClick={() => handleSocialLogin("kakao")} className="w-full py-3 rounded-full bg-yellow-400 text-black font-semibold hover:bg-yellow-500 transition">
                  카카오 로그인
                </button>
                <button disabled={loading} onClick={() => handleSocialLogin("google")} className="w-full py-3 rounded-full bg-red-500 text-white font-semibold hover:bg-red-600 transition">
                  구글 로그인
                </button>
                <button disabled={loading} onClick={() => handleSocialLogin("naver")} className="w-full py-3 rounded-full bg-green-600 text-white font-semibold hover:bg-green-700 transition">
                  네이버 로그인
                </button>
              </div>
              <button onClick={() => setShowSocialModal(false)} className="mt-5 w-full py-2 text-sm text-gray-600 hover:text-black">
                닫기
              </button>
            </div>
          </div>
